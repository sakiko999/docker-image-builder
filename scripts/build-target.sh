...
if [[ "$publish" == true ]] && ! state_needs_build "$target_id" "$upstream_tag" "$upstream_sha"; then
...
if [[ "$publish" == true ]]; then
  ...
  write_state "$target_id" "$repository" "$upstream_tag" "$upstream_sha" \
    "$image_repository" "$overlay_sha" "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
fi
