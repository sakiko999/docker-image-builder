...
base_image="local/cli-proxy-api-upstream:$UPSTREAM_SHA"

# 构建上游基础镜像，透传上游 Dockerfile 的 VERSION/COMMIT/BUILD_DATE 构建参数
# （上游二进制通过 ldflags 将其编译进 main.Version/Commit/BuildDate）
docker build --platform "$platform" \
  --file "$SOURCE_DIR/Dockerfile" \
  --build-arg "VERSION=$UPSTREAM_TAG" \
  --build-arg "COMMIT=$UPSTREAM_SHA" \
